    ...
    const updateHandle = (id) => {
        ...
        const delTodos = todos.map((item) => {
            if (item.id === id){
                ...
                //    todos.todo, todo 속성: state todo의 todo 입력된 값
            }
                return item;
        });
        setTodos([...delTodos]);
            ...
            {!todos.length 
                ...
                : todos.map((todo) => (
                    <li key={todo.id}>
                        ...
                        <input type='button' id='delete' value="삭제" onClick={() => deleteHandle(todo.id)}/>
                        <input type='button' id='update' value="수정" onClick={() => updateHandle(todo.id)}/>
                    </li>
            ))}
        </ul>
    </>
...
// <></>fragment : 레이아웃이 필요없는 빈 태그
// 디자인 요소 필요 없이 전체를 하나로 묶어주는 역할